...
exports.createCar = async (req, res)=>{
    ...
    const car = new Car({  
        ...req.body,
        image: req.file.path,
        id_owner: res.locals.auth._id
    });
    const places = await Place.findOne({place_number: req.body.place_number});
    try {
        const task =  Fawn.Task();
        const taches = await task.save('Car', car)
        .save('OwnerCar',
        {id_owner: res.locals.auth._id, id_car: car._id, id_place: places._id}
        )
        .run({ useMongoose: true })
